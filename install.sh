...
Finalizing npm install."
		prime_agent_run_quiet_with_animation_steps \
			...
			"Installing Prime Agent" \
			"$npm_install_details" \
			env $(npm_allow_remote_env) PRIME_AGENT_BOOTSTRAP_TOOLS_ON_INSTALL=1 PRIME_AGENT_BOOTSTRAP_KERNEL_ON_INSTALL=1 PRIME_AGENT_INSTALL_UV=1 npm install -g --prefix "$install_prefix" --no-fund --no-audit --loglevel=error --progress=false "$tarball_path"
	else
		npm_install_details="Preparing isolated install.
Linking command binaries.
Installing runtime packages.
