...
class _PrimeAgentUnavailableSkill:
    def __init__(self, name, error):
        ...
        self.__doc__ = f"Skill {name} is unavailable: {error}"

    async def run(self, *args, **kwargs):
        raise RuntimeError(
            f"Skill {self.__name__} is unavailable in this kernel. "
    ...
    def __repr__(self):
        return f"<unavailable skill {self.__name__!r}: {self._prime_agent_import_error}>"
...
export const buildXonshRlmBootstrapCode = buildRlmBootstrapCode;

export const xonshSchema = Type.Object({
	code: Type.String({
		description:
			"Xonsh code to execute in the persistent Xonsh REPL. Use the target project's own environment for project imports, tests, scripts, CLIs, and dependency checks instead of direct kernel imports.",
	}),
});
...
const BUSY_KERNEL_PROMPT = [
	"Interrupted Xonsh cell is still running",
...
const KERNEL_RESTART_NOTICE = [
	"<xonsh_kernel_reset>",
	"The Xonsh kernel was restarted after a previous interrupted cell kept running. Variables, imports, async tasks, and open resources from before the restart are no longer available; recreate them before using them.",
	"</xonsh_kernel_reset>",
].join("\n");

function createAbortError(): Error {
	return new Error("Xonsh execution aborted");
}
...
export type XonshToolInput = Static<typeof xonshSchema>;

export interface XonshToolDetails {
	durationMs?: number;
...
export interface XonshToolOptions {
	/** Xonsh binary or Python override. Must have prime-agent-runtime installed. */
	xonsh?: string;
	python?: string;
	...
	xonshSkills?: readonly PythonSkillRuntimeInfo[];
	...
	provisioner?: XonshKernelProvisioner;
}

/**
 * Owns the lazy create+start+runtime-bootstrap of one session's Xonsh kernel.
 *
 ...
 */
export class XonshKernelProvisioner {
	...
	constructor(
		...
		private readonly options?: Omit<XonshToolOptions, "provisioner">,
	) {}
	...
	private async startKernel(signal?: AbortSignal): Promise<KernelClient> {
		...
		try {
			...
			const skills = this.options?.xonshSkills ?? this.options?.pythonSkills;
			const bootstrapCode = buildRlmBootstrapCode(skills);
			const m = new ReplKernelManager({
				python: this.options?.xonsh ?? this.options?.python,
				cwd: this.cwd,
			...
			try {
				...
				this.emitStartupProgress("Starting Xonsh kernel...");
				...
				if (snapshotDir) {
					...
					this.emitStartupProgress("Restoring Xonsh state...");
				...
				this.emitStartupProgress("Preparing Xonsh runtime...");
				const bootstrap = await m.execute(bootstrapCode, {
					signal: startupSignal,
				});
				if (bootstrap.status !== "ok") {
					const details = [bootstrap.stderr, bootstrap.error?.traceback.join("\n")].filter(Boolean).join("\n");
					throw new Error(`Failed to initialize rlm runtime in the Xonsh kernel:\n${details}`);
				}
			} catch (error) {
...
async function executeWithBusyKernelChoice(
	...
	while (true) {
		...
		try {
			...
			if (action === "wait") {
				onWorkingMessage("Waiting for Xonsh kernel...");
				continue;
			}
			if (action === "kill") {
				onWorkingMessage("Restarting Xonsh kernel...");
...
export function createXonshToolDefinition(
	cwd: string,
	options?: XonshToolOptions,
): ToolDefinition<typeof xonshSchema, XonshToolDetails> {
	const provisioner = options?.provisioner ?? new XonshKernelProvisioner(cwd, options);

	return {
		name: "xonsh",
		label: "xonsh",
		description:
			"Execute Xonsh code in a persistent Xonsh REPL. Top-level `await` is supported. Variables, imports, and loaded data persist across calls, and are revived on a best-effort basis when a session is resumed (objects that cannot be serialized are dropped and reported). Run shell commands directly or with `bash('cmd')` / `await bash('cmd')`. Project imports, tests, scripts, CLIs, and dependency checks should run through the target project's own environment.",
		promptSnippet: "xonsh - persistent Xonsh REPL for code, state, and shell orchestration",
		// The kernel is single-threaded — pi must not run two xonsh calls in parallel within a batch.
		executionMode: "sequential",
		parameters: xonshSchema,
...
export function createXonshTool(cwd: string, options?: XonshToolOptions): AgentTool<typeof xonshSchema> {
	return wrapToolDefinition(createXonshToolDefinition(cwd, options));
}
