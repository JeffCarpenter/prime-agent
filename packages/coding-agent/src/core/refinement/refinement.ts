...
export function formatHarnessStateForPrompt(
	...
	options: {
		...
		/** Include Python/Xonsh REPL examples. `includeIpythonExamples` remains a legacy alias. */
		includeXonshExamples?: boolean;
	...
	const includeReplExamples =
		options.includeXonshExamples === true ||
		options.includeIpythonExamples === true ||
		(options.includeXonshExamples === undefined && options.includeIpythonExamples === undefined);
	...
	const lines = [
		...
		"",
		includeReplExamples
			? "Call contract: read each installed Python skill's SKILL.md and call its documented module function in the Python skill runtime (Xonsh or IPython REPL); do not assume a `.run` entrypoint. Use `<skill_import> ...` in shell when a CLI exists. Continual harness skill entries are Python REPL skills with an explicit Python `reference` and `arguments` contract. Spawn a continual harness subagent spec by composing a concise task prompt and calling `handle = await rlm('sub-task')`; admission returns immediately with `rlm_child_id`, `name`, `session_dir`, and `model`, never the child's answer. Results arrive only through explicit `agent_message` replies or files; children reply with `await agent_message.send(message, receiver_role='parent')`. Use `await rlm.list_subagents()` to recover direct child handles and `await agent_message.send(..., receiver_role='child', receiver_name=handle.name)` for follow-ups. Do not invent wrappers such as `call_skill(...)`, `run_subagent(...)`, or named subagent registries."
	...
	for (const kind of Object.keys(state.entries) as RefinementKind[]) {
		...
		if (kind === "subagent" && entries.length > 0 && includeReplExamples) {
			lines.push(
