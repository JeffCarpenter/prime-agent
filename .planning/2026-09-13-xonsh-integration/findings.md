...
## Key Architectural Surprises & Insights (Post-Grep)
During the exhaustive architectural mapping (where we found 954 matches for `ipython` across 123 files), several fascinating architectural constraints and mechanisms were discovered that directly impacted our Xonsh integration:

1. **The Pervasiveness of the `ipython` String:** It is not merely a tool ID. It is a structural assumption woven into the session core. Goals explicitly checked for it and threw exceptions if missing; UI components (`IPythonCellComponent`), session state tags (`<ipython_state>`), and fallback logic all depended on it.
2. **Late Agent Messages Recovery:** `agent-session.ts` maintains a highly specialized caching mechanism (`_lateIpythonSentAgentMessages`). If the Python kernel emits an asynchronous `xonsh_sent_agent_message` JSON payload right as an LLM turn ends, the session stashes it and injects it into the *next* turn. This ensures the LLM never loses sync with background process output, meaning our `xonsh` tool had to explicitly wire into this exact lifecycle to survive.
...
### SDK Full-Control & Default Xonsh Tool Findings (2026-09-15)
- **Default Tool Surface in `createAgentSession`**: In `packages/coding-agent/src/core/sdk.ts`, when `options.tools` is omitted, `initialActiveToolNames` defaults to `["xonsh"]` and `allowedToolNames` is `undefined`. `AgentSession` instantiates its own `XonshKernelProvisioner` and registers `xonsh` in `_baseToolDefinitions`. Omitting `tools` activates `xonsh` out-of-the-box.
- **`customTools` vs `process.env.PYTHONPATH`**: `AgentSession` builds its internal kernel provisioner with `_rlmKernelEnv()`, which inherits process environment variables but does not bundle local monorepo source paths (`prime-agent-runtime/src`). Setting `process.env.PYTHONPATH` allows `AgentSession` to run real Xonsh execution without passing `customTools` or custom provisioners.
- **Proving `tools` and `customTools` are Unnecessary**: Proved that `createAgentSession` executes real Xonsh cells out-of-the-box when neither `tools` nor `customTools` is passed, relying entirely on the built-in default `xonsh` tool and `AgentSession`'s internal `XonshKernelProvisioner`.
- **`AgentSession.getKernelProvisioner` API**: Added typed overloaded method `getKernelProvisioner(name: "ipython" | "xonsh"): IpythonKernelProvisioner | XonshKernelProvisioner | undefined` on `AgentSession`, enabling inspection of provisioners and `provisioner?.hasRunningKernel`.
- **Async `AgentSession.dispose`**: Made `AgentSession.dispose()` async (`Promise<void>`) to ensure kernel provisioners (`_ipythonKernelProvisioner` and `_xonshKernelProvisioner`) and callbacks are cleanly disposed and awaited during session teardown.
- **Faux Provider Auth Requirements**: Even in testing with in-memory faux providers, `AgentSession._validateCanStartAgentRun` gates turn execution on `authStorage.hasKey(model.provider)` and `modelRegistry.hasConfiguredAuth(model)`. `authStorage.setRuntimeApiKey(model.provider, "faux-key")` and `modelRegistry.registerProvider(...)` must be configured before `session.prompt()`.
- **`strictFunctionTypes` Parameter Contravariance**: `customTools?: ToolDefinition[]` on `CreateAgentSessionOptions` defaults `TSchema` to `TSchema`, expecting `renderCall` arguments of type `unknown`. Concrete definitions like `createXonshToolDefinition` type `args: { code: string }`, requiring an explicit cast (`as unknown as ToolDefinition`) under `strictFunctionTypes`.
- **Integration Coverage**: Verified end-to-end in `packages/coding-agent/test/suite/xonsh-repl.integration.test.ts` with cell execution, state recovery across errors, process capture (`$()`), environment manipulation, and full-control SDK session execution without explicit tools.
